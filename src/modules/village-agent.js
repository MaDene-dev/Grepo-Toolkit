...
class VillageAgent {
  ...
  stop() {
    ...
    if (this.stats_writer) this.stats_writer.updateStatus("stopped");
  ...
  async run() {
    ...
    try {
      ...
      // Schrijf stats na elke ronde naar dashboard
      if (this.stats_writer) {
        this.stats_writer.recordSession(this.stats, this.history);
      }
